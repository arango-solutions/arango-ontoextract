"use client";

export type LensType = "semantic" | "confidence" | "curation" | "diff" | "source";

interface LensToolbarProps {
  activeLens: LensType;
  onLensChange: (lens: LensType) => void;
  selectedOntologyId: string | null;
  selectedOntologyName?: string;
}

const LENSES: { id: LensType; label: string; shortcut: string }[] = [
  { id: "semantic", label: "Semantic", shortcut: "1" },
  { id: "confidence", label: "Confidence", shortcut: "2" },
  { id: "curation", label: "Curation", shortcut: "3" },
  { id: "diff", label: "Diff", shortcut: "4" },
  { id: "source", label: "Source", shortcut: "5" },
];

export default function LensToolbar({
  activeLens,
  onLensChange,
  selectedOntologyId,
  selectedOntologyName,
}: LensToolbarProps) {
  return (
    <header className="h-12 border-b border-gray-200 bg-white flex items-center px-4 gap-4 flex-shrink-0">
      {/* Logo */}
      <div className="flex items-center gap-2 mr-4">
        <span className="text-base font-bold tracking-tight text-gray-900">
          AOE
        </span>
        <span className="text-xs text-gray-400 hidden sm:inline">
          Workspace
        </span>
      </div>

      {/* Lens segmented control */}
      <div className="flex items-center bg-gray-100 rounded-lg p-0.5 gap-0.5">
        {LENSES.map((lens) => (
          <button
            key={lens.id}
            onClick={() => onLensChange(lens.id)}
            disabled={!selectedOntologyId}
            title={`${lens.label} lens (${lens.shortcut})`}
            className={`px-3 py-1.5 text-xs font-medium rounded-md transition-all duration-150
              ${
                activeLens === lens.id && selectedOntologyId
                  ? "bg-white text-gray-900 shadow-sm"
                  : selectedOntologyId
                    ? "text-gray-500 hover:text-gray-700 hover:bg-gray-50"
                    : "text-gray-300 cursor-not-allowed"
              }
            `}
          >
            {lens.label}
          </button>
        ))}
      </div>

      <div className="flex-1" />

      {/* Selected ontology indicator */}
      {selectedOntologyId && (
        <div className="flex items-center gap-2 text-xs text-gray-500">
          <span className="inline-block h-2 w-2 rounded-full bg-green-400" />
          <span className="font-medium text-gray-700 truncate max-w-[200px]">
            {selectedOntologyName ?? selectedOntologyId}
          </span>
        </div>
      )}

      {/* Settings placeholder */}
      <button
        className="p-1.5 rounded-md text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition-colors"
        title="Settings"
        aria-label="Settings"
      >
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
          <path
            strokeLinecap="round"
            strokeLinejoin="round"
            d="M9.594 3.94c.09-.542.56-.94 1.11-.94h2.593c.55 0 1.02.398 1.11.94l.213 1.281c.063.374.313.686.645.87.074.04.147.083.22.127.325.196.72.257 1.075.124l1.217-.456a1.125 1.125 0 011.37.49l1.296 2.247a1.125 1.125 0 01-.26 1.431l-1.003.827c-.293.241-.438.613-.43.992a7.723 7.723 0 010 .255c-.008.378.137.75.43.991l1.004.827c.424.35.534.955.26 1.43l-1.298 2.247a1.125 1.125 0 01-1.369.491l-1.217-.456c-.355-.133-.75-.072-1.076.124a6.47 6.47 0 01-.22.128c-.331.183-.581.495-.644.869l-.213 1.281c-.09.543-.56.94-1.11.94h-2.594c-.55 0-1.019-.398-1.11-.94l-.213-1.281c-.062-.374-.312-.686-.644-.87a6.52 6.52 0 01-.22-.127c-.325-.196-.72-.257-1.076-.124l-1.217.456a1.125 1.125 0 01-1.369-.49l-1.297-2.247a1.125 1.125 0 01.26-1.431l1.004-.827c.292-.24.437-.613.43-.991a6.932 6.932 0 010-.255c.007-.38-.138-.751-.43-.992l-1.004-.827a1.125 1.125 0 01-.26-1.43l1.297-2.247a1.125 1.125 0 011.37-.491l1.216.456c.356.133.751.072 1.076-.124.072-.044.146-.086.22-.128.332-.183.582-.495.644-.869l.214-1.28z"
          />
          <path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
        </svg>
      </button>
    </header>
  );
}
