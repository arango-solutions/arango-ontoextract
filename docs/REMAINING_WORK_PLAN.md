...
| Constraints (§6.14) | **PR 1 + PR 2 shipped (v0.4.0-dev)** | Extraction (PR 1) → OWL restriction import (PR 2) → materialization → API → temporal → rule engine alignment all shipped (I.1, I.2, I.3, I.5, I.9, plus rule-engine schema reconciliation). Remaining: SHACL shapes import (I.4 → PR 3), workspace UI (I.6 + I.7 → PR 4), export (I.8 → PR 5). |
...
**Exit Criteria (overall Stream 3):** Constraints extractable, importable (OWL + SHACL), displayable in workspace, and exportable (OWL Turtle + SHACL shapes graph). Pending PRs 3–5.
