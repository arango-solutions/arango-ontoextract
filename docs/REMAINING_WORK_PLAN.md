...
| Image-Aware Extraction (§6.1, §6.2, §6.11) | **Complete (Stream 13 IMG.1–IMG.8 + OpenAI Vision adapter)** | Visual asset inventory (PPTX picture/chart shapes, PDF image blocks, scanned-only pages); labeled placeholders + alt-text + caption markers in chunk text; `chunk_kind` + per-chunk `visual_assets` propagated through chunking; `visual_heavy_presentation` strategy + `tier1_visual_aware` prompt; `VisualCaptionProvider` adapter with concrete `OpenAIVisionCaptionProvider` (auto-loaded lazily on `visual_caption_provider="openai_vision"`) + no-op default + per-doc cap; orphan-risk warning persisted to `extraction_runs.stats.warnings` when visual-heavy input correlates with parent-less classes; regression coverage in `tests/unit/test_visual_extraction_regression.py` and `tests/unit/test_visual_captions_openai.py`. Optional on-prem Tesseract adapter remains future work. |
...
| I.7 | Constraint actions in workspace context menu | Frontend | **Blocked** | Curator approve / reject / edit of constraints -- gated on the mutation API. Will reopen once the mutation endpoint lands; PR 4 ships read-only display so curators can SEE constraints today. |
...
* **I.7 explicitly deferred**: PR 4 ships read-only display only. The original PR description considered context-menu approve / reject actions, but those need a mutation endpoint (POST `/library/{id}/constraints/{key}/approve` etc.) that doesn't exist yet. Adding actions without the backing API would be wiring a button to nothing -- a "wiring-over-deletion" violation in reverse. Reopens when the mutation API lands.
* **Test coverage added**:
...
**Exit Criteria (overall Stream 3):** ✓ Constraints extractable (PR 1), importable from OWL (PR 2), importable from SHACL (PR 3), displayable in the workspace (PR 4), exportable to OWL Turtle + SHACL shapes graph (PR 5). Stream 3 v1 complete. Only deferred item is I.7 (curator approve / reject context-menu actions) which remains blocked on a mutation API that doesn't yet exist.
...
| IMG.4 | OCR / vision-caption adapter | Backend | 1 day | **DONE** — `VisualCaptionProvider` abstract base, `NoOpCaptionProvider` default, `register_caption_provider`/`get_caption_provider` registry with lazy-import for optional adapters. Concrete `OpenAIVisionCaptionProvider` in `app/services/visual_captions_openai.py` activates on `settings.visual_caption_provider="openai_vision"`; uses `settings.openai_api_key` + `visual_caption_openai_model` (default `gpt-4o-mini`), retries on transport errors with exponential backoff, treats missing key / empty content / `no_recognizable_content` as structured failures so ingestion falls back cleanly. Tesseract on-prem adapter remains tracked as an optional follow-up (boundary is in place; no host dependency by default). |
