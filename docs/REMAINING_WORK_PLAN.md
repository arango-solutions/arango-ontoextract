...
**Remaining for v1.0.0 (small tail):** Stream 12 T6 WTW switch profile — `/effective` (the endpoint the canvas actually loads) now carries per-stage `ms_*` telemetry alongside the existing `/edges` + `/classes` timing, so this is one real profile capture on a 1000+ class ontology away from closing. The `/edges` + `/effective` pagination that profile may justify stays deferred until the capture lands. (Stream 3 I.7 curator approve/reject/edit — previously listed here as blocked — shipped in commit `fbb72db`; Stream 4 was never formally short of its exit criteria, see below.)
**Post-v1.0:** Stream 4 RAG-benchmark comparison UI (optional — never a formal Stream 4 task; Q.1–Q.5 shipped and the stream's exit criteria are met); Stream 8 editor panels (semantic zoom, edge bundling, property matrix, restriction editor, namespace manager, validation console); Stream 9 unified-storage spike; legacy-route removal. See the refreshed execution order below.
...
| # | Task | Type | Estimate | Description |
|---|------|------|----------|-------------|
| T6 | WTW switch ~8-9s investigation | Backend | TBD (≤ 1 day after first profile) | **Telemetry gap closed (v0.4.0-dev).** Stage-level logs existed on `/edges` + `/classes` (T3) but the canvas does **not** call those — it loads `GET /{id}/effective?include=summary`, which had no per-stage timing. `compute_effective_ontology` now logs per-stage `ms_*` (meta-snapshot / closure / fetch / project / conflicts / etag / total) with the same message-plus-`extra` pattern as `list_ontology_edges`, so a WTW switch on a 1000+ class ontology now surfaces exactly where the cost goes. **Remaining:** capture the breakdown from one real switch on a large ontology and act on the dominant stage (likely candidates: the 3-AQL closure fetch on a deep import chain, or `_detect_conflicts`' O(n²) label/URI grouping + DFS cycle walk on the merged set). Pagination on `/edges` + `/effective` stays deferred until that capture justifies the "progressive load + relayout" effort. |
...
REMAINING for v1.0.0 (small tail):
  - Stream 12: T6 WTW switch profile. /effective now carries per-stage ms_*
               telemetry (matching /edges + /classes), so all that remains is to
               capture one real switch on a 1000+ class ontology and act on the
               dominant stage. /edges + /effective pagination stays deferred until
               that capture justifies it.
  → v1.0.0 Release

  (Closed since v3.3: Stream 3 I.7 curator approve/reject/edit shipped in commit
   `fbb72db`; Stream 4 confirmed feature-complete — its exit criteria never
   included the RAG benchmark UI, which is now post-v1.0.)
