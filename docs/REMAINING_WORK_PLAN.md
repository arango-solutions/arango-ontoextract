...
| **Phase 2b: Canvas & Extraction** | H.15 (DONE), H.16 (DONE), H.17 | ~1 day remaining | Phase 2a |

**Parallelization:** All frontend work for Phase 2b has landed; H.17 is a backend-only task (effective-context serializer + extraction-prompt update) and proceeds independently.
...
**Phase 2a + H.15/H.16 status (v0.4.0-dev):** the effective-graph data layer (H.12), merge-conflict detection (H.13), canvas rendering of imported entities (H.15), and drag-and-drop import composition (H.16, with undo-toast surface) are shipped. Only H.17 (import-aware extraction prompts) remains on Stream 1; everything in the exit criteria above except the prompt-side of the composed-ontology workflow is now met.
