...
| Constraints (§6.14) | **PR 1 + PR 2 + PR 3 + PR 4 shipped (v0.4.0-dev)** | Extraction (PR 1) → OWL restriction import (PR 2) → SHACL shapes import (PR 3) → materialization → API → temporal → rule engine alignment + workspace UI display (PR 4) all shipped (I.1–I.6, I.9, plus rule-engine schema reconciliation and SHACL/OWL cross-vocab combination). Remaining: curator approve / reject mutation actions (I.7 → blocked on mutation API), export (I.8 → PR 5). |
...
**Exit Criteria (overall Stream 3):** Constraints extractable, importable (OWL + SHACL), displayable in workspace (PR 4 ✓), and exportable (OWL Turtle + SHACL shapes graph -- PR 5 pending). Pending: PR 5 export, I.7 mutation actions (blocked on mutation API).
