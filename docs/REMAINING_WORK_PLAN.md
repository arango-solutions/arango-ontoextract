...
| Imports, Composition & Dependencies (§6.15, §6.8.8–8.16) | **Phase 0 + Phase 1 done (v0.4.0-dev); Phase 2 pending** | `owl:imports` edge tracking, imports CRUD, `ontology_imports` named graph, standard ontology catalog (`/ontology/catalog` + bundled DCMI sample), `GET /imports-graph` DAG endpoint, cascade-on-delete impact, base-ontology selector on extraction, OWL exports preserving `owl:imports`, workspace catalog-browser overlay, workspace imports-dependency overlay (DAG canvas + library deep-link), and three Visualizer saved queries are all shipped. Phase 2 remaining: effective-graph API (H.12), import conflict detection (H.13), distinct rendering of imported entities (H.15), drag-and-drop import composition (H.16), import-aware extraction prompts (H.17). |
...
**Duration:** Phase 0 + Phase 1 (1a + 1b) complete in v0.4.0-dev. Phase 2 (composition / effective graph / drag-and-drop / extraction prompts) remaining.
...
**Parallelization:** Phase 2a (effective graph API) can run independently of Phase 2b once started. Phase 2b's canvas work (H.15) depends on the effective-graph response shape stabilising in Phase 2a; H.16/H.17 can begin in parallel as soon as H.12 lands.
...
**Phase 1 status (v0.4.0-dev):** all of the above except "effective graph rendered in canvas with visual distinction" are met by Phase 0 + Phase 1. The remaining bullet is the Phase 2 goal (H.12 + H.15).
