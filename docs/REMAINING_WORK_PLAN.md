...
| Production Ops (§8.5) | **Tracing + alerting + prod compose done** | Stream 7 PR 1 (TTL GC + visualizer auto-install), PR 2 (OpenTelemetry tracing), PR 3 (alerting + prod docker-compose hardening + monitoring profile). PR 4 (benchmarks + README) remaining. |
