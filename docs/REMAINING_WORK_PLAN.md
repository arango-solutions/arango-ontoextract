...
| Imports, Composition & Dependencies (§6.15, §6.8.8–8.16) | **Phase 0 + Phase 1 + Phase 2a done (v0.4.0-dev); Phase 2b pending** | `owl:imports` edge tracking, imports CRUD, `ontology_imports` named graph, standard ontology catalog (`/ontology/catalog` + bundled DCMI sample), `GET /imports-graph` DAG endpoint, cascade-on-delete impact, base-ontology selector on extraction, OWL exports preserving `owl:imports`, workspace catalog-browser overlay, workspace imports-dependency overlay (DAG canvas + library deep-link), three Visualizer saved queries, effective-graph API (`GET /{id}/effective` with inline conflicts + ETag), and merge-conflict detection (duplicate URI / duplicate label / subclass cycle via import) are all shipped. Phase 2b remaining: distinct rendering of imported entities (H.15), drag-and-drop import composition (H.16), import-aware extraction prompts (H.17). |
...
**Parallelization:** Phase 2b's canvas work (H.15) consumes the effective-graph response shape stabilised in Phase 2a; H.16 (drag-and-drop composition) and H.17 (import-aware extraction prompts) can begin in parallel against the same API contract.
...
**Phase 2a status (v0.4.0-dev):** the effective-graph data layer (H.12) and merge-conflict detection (H.13) are shipped. The remaining exit-criteria bullet — "effective graph rendered in canvas with visual distinction for imported entities" — is now strictly an H.15 UI task on top of the existing API.
