...
| IMG.8 | Regression fixtures and tests | Backend | 1 day | Synthetic PPTX with image-only taxonomy and title-only hierarchy; scanned/image-only PDF fixture; tests for inventory, placeholder mode, OCR/caption injection, prompt rendering, and orphan-risk warning. (Title-only PPTX + picture-bearing PPTX fixtures already shipped; image-only-taxonomy + scanned-PDF fixtures remain.) |
