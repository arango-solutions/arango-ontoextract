...
| Q13 | Multi-Domain Document Strategy | High | When an extraction run produces content spanning multiple domains (detected per FR-2.15), the system must decide which of three strategies to apply — and we have not committed to one yet. **Option A: Mixed-into-single-ontology** — keep one target ontology; tag each class with `domain_tag`; cross-domain edges remain in the same graph. *Pro:* simplest; preserves cross-domain relationships natively. *Con:* the ontology mixes concerns (a Financial-Services ontology with Supply-Chain leaf classes is hard to release / re-use). **Option B: Split-into-per-domain-ontologies** — create N staging ontologies, one per detected domain; cross-domain edges become `imports` + `extends_domain` references; curator accepts/rejects per-domain. *Pro:* cleanest ontologies; reusable per-domain. *Con:* cross-domain relationships now span ontology boundaries, requiring effective-graph computation (FR-15.8) at query time. **Option C: Umbrella ontology that imports per-domain ontologies** — create N per-domain ontologies (Option B) *and* an umbrella ontology that `owl:imports` all of them; the umbrella is the curator's working surface; per-domain ontologies are reusable independently. *Pro:* combines B's reusability with A's single-canvas curation; aligns with the composition model already shipped in Stream 1 (FR-15.7). *Con:* three places to look (umbrella + N domains) for any one class; UI must clearly indicate whose-class-is-this. **Open sub-questions:** (a) Which strategy is the default — auto-applied — vs. always curator-prompted? (b) For an existing target ontology that receives a multi-domain document, do we coerce to Option A (preserve target), suggest a split (Option B/C), or block until the curator chooses? (c) How do we track that a document contributed to multiple ontologies — is `extracted_from` 1:N already, or does it need extending? (d) Domain-detection signal: embedding-cluster on chunk vectors, LLM classification against a domain taxonomy, or both with a confidence threshold? **Recommendation pending design spike** — likely Option C as default with Option A as a curator override, because it composes with the imports machinery we already have. |
